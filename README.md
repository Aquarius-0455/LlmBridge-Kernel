...
`LlmBridge` 是一个超轻量级、零外部依赖的**大语言模型（LLM）流式调用与故障降级桥接网关**。它旨在为开发者提供一个拥有可视化调试面板的网关代理，支持多通道轮询、任务场景别名路由、流式断线自动降级（Fallback Chain）以及多模态图片大图等比压缩上传。
...
*   **🗺️ 任务场景路由 (Routing Mappings)**：支持为不同的工作流场景（如 `Planning`、`Execution`、`Translation` 等）绑定特定模型，聊天时只需指定别名即可自动路由。
...
### ⚙️ 对话调试微调设置 (iOS Slider & Capsule Selector)
